...
func (tws *TwitchService) GetUser(ctx context.Context, token, id string) (*models.GetUserInfoResponse, error) {
	...
	if userInfo.Data[0].UserID != id && userInfo.Data[0].Login != id {
		return nil, errors.Errorf("invalid reponse data, give %s, got id %s, login %s",
			id, userInfo.Data[0].UserID, userInfo.Data[0].Login)
	}
